...
function App() {
  ...
  return (
    <Box
      sx={{
        display: "grid",
        gridAutoRows: sites.length ? "80px 70px auto" : "1fr 1fr",
        gridAutoColumns: "1fr",
        minHeight: "90vh",
    ...
    >
      ...
      >
        ...
        <ColorChangingText text="BAR_" color="#7211d9" fontSize="5em" />
      </Box>
      <Box sx={{ width: "100%" }}>
        <FormProvider {...form}>
          <Input
      ...
      {sites.length ? (
        ...
        >
          {sites.map((site: any, index: number) => (
            <SiteResult key={site.id} index={index} {...site} />
          ))}
        </Box>
      ) : null}
    </Box>
